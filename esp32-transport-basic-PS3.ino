/*
  ...
  Buttons O= 2         Axis L : atas = - 502
      X= 1                     kiri   = - 508
  kotak = 4                      kanan  = 512
  Segitiga = 8                   bawah = 512

  Dpad
  Panah atas = 1      Axis R : atas = - 502
  Panah bawah = 2               kiri = - 508
  Panah kiri = 8                kanan = 512
  Panah kanan = 4               bawah = 512

  L1 = 0×10                      R2 throttle :1020 (analog?)
  L2 = 0×20                      L2 brake    :1020 (analog?)
  L1+L2 = 0×30

  Buttons
  O = ( berputar ditempat ke arah kanan )

  |_| kotak =  (berputar ditempat ke arah kiri)

  /_\ Segitiga = ( maju )
  X = ( mundur )

  Dpad
  Panah atas ( menaikan lengan ke tinggi )
  Panah bawah ( menurunkan lengan ke rendah )
  Panah kiri ( menaikan lengan robot sedang )
  Panah kanan ( menaikan lengan robot sedang )

  L1 ( mencapit sekaligus melepaskan )
  L1+L2 (cit code)

  Axis L & R (-512 sampai +512, center 0 +- 5)
  Atas (maju)
  Kiri ( berbelok/memutar di tempat ke arah kiri )
  Kanan ( berbelok/memutar ditempat ke arah kanan )
  Bawah ( mundur )

  R Throttle = maju, 0-1024
  L Throttle = mundur, 0-1024
*/
/*
...
#define SANGKAT_HIGH 0
#define SANGKAT_MID 90
#define SANGKAT_LOW 180
#define SANGKAT_DELTA 5
  ...
  for (int i = 0; i < ncontroller; i++) {
    if (P[i]->isGamepad()) {
      ...
      if (abs(yaxisdata) > abs(y2axisdata)) {
        ...
        ydata = (y2axisdata + 512) / 4; //jadikan skala 128 dan berpusat di 128
      }
      if (abs(xaxisdata) > abs(x2axisdata)) {
        xdata = (xaxisdata + 512) / 4; //jadikan skala 128 dan berpusat di 128
      }
      else {
        xdata = (x2axisdata + 512) / 4; //jadikan skala 128 dan berpusat di 128
      }
      if (throttle > 50) {
        ydata = throttle / 8 + 128; //dari 0-1024 jadi 128-255 (cuma bisa maju)
      }
      else if (brake > 50) {
        ydata = 128 - (brake / 8); //dari 0-1024 jadi 128-0 (mundur)
      }
      if (buttons & BUTTON_O) { //putar kanan
        if (adagyro) { //putar kanan 90 derajat
          if (PSbstate == false) {
            PSbstate = true;
            putardelta(M_PI_2);
          }
...
void setup() {
  ...
  //init bluepad gamepad host
  BP32.setup(&onPSconnect, &onPSdisconnect);
