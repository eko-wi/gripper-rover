...
/*
   ...
   output gyro: -PI sampai +PI, putar kanan --> makin besar
   kontrol dengan ibus (lewat serial, karena serial2 (pin 16 17) sudah dipakai motor):
   channel 1:(analog) 991 = putar kiri 90, 1500 = no action, 2000 = putar kanan 90
   channel 2:(analog) 991=kiri, 1500=tengah, 2000=kanan
   channel 3:(analog) 2000 = maju, 1500=diam, 988 = mundur
   channel 4:(switch) grip: 988=buka, 2011=tutup
   channel 5:(switch) posisi arm: 2000=tinggi, 1500=mid, 988=rendah
...
//variabel-variabel ibus
#define IBUS_MAGIC1 0x20
#define IBUS_MAGIC2 0x40
#define NCHANNELS 14
#define DEADBAND 12
int ibdata[NCHANNELS * 2];
int idata[NCHANNELS];
int nbytes = 0;
int ichecksum = 0;
...
PIDController depan(70, 1.2, 10000, 80, 50);  //set motor value dalam persen, maks 100
...
void prosesdata(Stream &S) {
  ...
  switch (state) {
    ...
    case IBUS_BYTE2:
      ...
      break;
    case IBUS_BYTES:
      ibdata[nbytes] = S.read();
      nbytes++;
      if (nbytes == NCHANNELS * 2) {
        nbytes = 0;
        //Serial.print("idata:");
        for (int i = 0; i < NCHANNELS; i++) {
          idata[i] = ibdata[i * 2] + (ibdata[i * 2 + 1] << 8);  //byte kecil dulu
          //Serial.print(idata[i]);
          //Serial.print(' ');
        }
        //Serial.print("xy:");
        //proses data di sini...
        //1000-2000 jadi 0-256
        xdata = idata[1] * 32 / 125 - 256;  //idata[1]*256/1000 - 256;
        ydata = idata[2] * 32 / 125 - 256;
        xdata = constrain(xdata, 0, 255);
        ydata = constrain(ydata, 0, 255);
        if (abs(ydata - 128) < DEADBAND) ydata = 128;
        if (abs(xdata - 128) < DEADBAND) xdata = 128;
        x2data = 128;
        //1000-2000 langsung jadi mikrosekon posisi arm
        sdata = idata[4];
        //perbesar skala gerakan servo
        sdata = (sdata - 1500) * 19 / 10 + 1500;
        //Serial.print(xdata); Serial.print(" ");
        //Serial.print(ydata); Serial.print(" ");
        //Serial.println(sdata);
        armpos.setrate(17);  //1000 poin (180 derajat) per 1000 milisekon
        s_angkat.writeMicroseconds(armpos.update(sdata));
        //grip: <1200 = buka, >1800 = tutup
        if (idata[3] > 1800) {
          s_grip.write(SGRIP_ANGLE_HOLD);
        } else if (idata[3] < 1200) {
          s_grip.write(SGRIP_ANGLE_OPEN);
        }
        if (putar90state == 0) {
          if (idata[0] < 1350) {  //putar kiri 90
            putar90state = 1;
            putardelta(-M_PI_2);
            Serial.println("L");
          } else if (idata[0] > 1650) {  //putar kanan 90
            putar90state = 1;
            putardelta(M_PI_2);
            Serial.println("R");
          }
        } else {  //masih ada sisa command sebelumnya, tunggu sampai kembali ke 1500
          if (idata[0] > 1400 && idata[0] < 160) {
            putar90state = 0;
          }
        }
        motorenable = 1;
        prosesoutput();
        ledcWrite(PWMC2, 255 - fabs(powermaju * 2.55));
        state = IBUS_CHECKSUM1;
      }
      break;
    case IBUS_CHECKSUM1:
      ichecksum = S.read();
      state = IBUS_CHECKSUM2;
      break;
    case IBUS_CHECKSUM2:
      j = S.read();
      ichecksum |= (j << 8);
...
void loop() {
  ...
  */
  ...
  if (t - tlastcommand > 120) {
    //Serial.println("timeout, stopmovement");
    stopmovement();
  }
